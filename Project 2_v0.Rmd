...
Our goal is to present to our boss a better understanding of which combination of factors most accurately predict the acidity levels in our company's manufactured beverages as measured by pH so that they can optimize the manufacturing process to keep up with stricter regulations. (PB)
...
* A Detailed technical report to be reviewed by an outside consultant 
...
We can also see from the histogram plots of individual variables that CarbPressure1, CarbPressure2, CarbTemp, CarbVol, FillOunces, PCVol and pH seem to be nearly normally distributed. We also see that AirPress, OxyFiller, PSC, PSCFill and Temp are left skewed while UsageCont seems to be right skewed. Interestingly, MNFFlow has negative values. Many of the variables seem to exhibit sparcity suc AlchRel, BowSetpoint, CarbFlow, FillerLevel, FillerSpeed, FillPressure, HydPress, HydPress2, HydPress3, MFR, MNFFlow, PressureSetpoint, PresVac and PSCCo2. (PB) 

```{r Histograms, echo=FALSE, message=FALSE, warning=FALSE}

# plot distributions of each independent variables (PB)
beverages_d %>%
  keep(is.numeric) %>%
  gather() %>%
  ggplot(aes(value)) + 
